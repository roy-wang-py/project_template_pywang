...
SHA1::SHA1()
{
    sha1_details::reset(digest, buffer, transforms);
}
...
void SHA1::update(std::istream &is)
{
    while (true)
    {
        char sbuf[sha1_details::BLOCK_BYTES];
        is.read(sbuf, sha1_details::BLOCK_BYTES - buffer.size());
        buffer.append(sbuf, (std::size_t)is.gcount());
        if (buffer.size() != sha1_details::BLOCK_BYTES)
        {
            return;
        }
        uint32_t block[sha1_details::BLOCK_INTS];
        sha1_details::buffer_to_block(buffer, block);
        sha1_details::transform(digest, block, transforms);
        buffer.clear();
    }
}
...
std::string SHA1::final()
{
    ...
    uint64_t total_bits = (transforms*sha1_details::BLOCK_BYTES + buffer.size()) * 8;

    /* Padding */
    buffer += (char)0x80;
    size_t orig_size = buffer.size();
    while (buffer.size() < sha1_details::BLOCK_BYTES)
    {
        buffer += (char)0x00;
    }
    ...
    if (orig_size > sha1_details::BLOCK_BYTES - 8)
    {
        sha1_details::transform(digest, block, transforms);
        for (size_t i = 0; i < sha1_details::BLOCK_INTS - 2; i++)
        {
            block[i] = 0;
        }
    }
    ...
    block[sha1_details::BLOCK_INTS - 1] = (uint32_t)total_bits;
    block[sha1_details::BLOCK_INTS - 2] = (uint32_t)(total_bits >> 32);
    sha1_details::transform(digest, block, transforms);
